# Unity 6000.3.12f1 LTS
公開日時: Wed, 25 Mar 2026 11:55:21 GMT
https://unity.com/releases/editor/whats-new/6000.3.12f1

# 6000.3.12f1 の既知の問題

- `6000.2.0a10`: プロジェクトを初めて開いたときに TexturesD3D12::CreateTextureInternal() でクラッシュする。
    ([uum-135024](https://issuetracker.unity3d.com/issues/crash-on-checkdevicestatus-when-opening-a-project-for-the-first-time))

- `6000.2.0b1`: DisableTypeTreeを使用したAsset BundleからVFXオブジェクトをロードする際、"CachedReader::OutOfBoundsError "でクラッシュする。
    ([uum-137538](https://issuetracker.unity3d.com/issues/crash-on-cachedreader-outofboundserror-when-loading-vfx-object-from-an-asset-bundle-with-disabletypetree))

- `6000.3.0a3`: Umbra::QueryExt::queryStaticShadowCasters で、Near Clipping Planes の最小値でカスタム Reflection Probe をベイクするとクラッシュする。
    ([uum-137250](https://issuetracker.unity3d.com/issues/crash-on-umbra-queryext-querystaticshadowcasters-when-baking-a-custom-reflection-probe-with-the-minimum-near-clipping-planes-value))

- `6000.6.0a1,6000.3.11f1,6000.4.0b12,6000.5.0a9`: プレハブオブジェクトのAPIを通してRigidBody Interpolationの値を変更すると、PhysicsCommands::PhysX::BodyGetFlagsでクラッシュします。
    ([uum-137716](https://issuetracker.unity3d.com/issues/crash-on-physicscommands-physx-bodygetflags-when-changing-rigidbody-interpolation-value-through-api-for-prefab-objects))

- `コアランタイム`: Entities ContentManagementのGUIDが不安定で、Playerが異なるディレクトリにビルドされるたびに異なってしまう。
    ([uum-129944](https://issuetracker.unity3d.com/issues/entities-contentmanagement-guids-are-unstable-and-become-different-each-time-a-player-is-built-to-a-different-directory))

- `メタル`: コマンドバッファのタイムアウトエラー後にゲームがフリーズする
    ([uum-125778](https://issuetracker.unity3d.com/issues/metal-game-freezes-after-command-buffer-timeout-error-1))

- `Metal`: プレイモードのエディタで最小プロジェクトで macOS が吃音になる
    ([uum-85256](https://issuetracker.unity3d.com/issues/macos-stutters-in-a-minimal-project))

ConfigurableJoint が JointProjectionMode.PositionAndRotation を使用し、接続された Rigidbody がキネマティックから非キネマティックに切り替わったときに、ConstraintProjectionTree::projectionTreeBuildStep でクラッシュする。
    ([uum-135394](https://issuetracker.unity3d.com/issues/crash-on-constraintprojectiontree-projectiontreebuildstep-when-configurablejoint-uses-jointprojectionmode-dot-positionandrotation-and-connected-rigidbody-switches-from-kinematic-to-non-kinematic))

新しいリリースには、最新のURPテンプレート・バージョンとHDRPテンプレート・バージョンが含まれていません。
    ([uum-137426](https://issuetracker.unity3d.com/issues/new-releases-are-shipped-without-the-newest-urp-and-hdrp-template-versions))

DirectX 12の排他的フルスクリーンモードでフォーカスを切り替えるとプレイヤーがクラッシュする問題
    ([uum-134743](https://issuetracker.unity3d.com/issues/player-crash-when-switching-focus-in-exclusive-fullscreen-mode-with-directx-12))



# 6000.3.12f1 リリースノート

## 機能

- `UIツールキット`: UIToolkit Test Frameworkのワールドスペースコンテンツの`simulate.Click`をサポートした。



## 改良点

- `ドキュメント`: Memory Profiler パッケージへの直接リンクを追加。

- `ドキュメント`: メモリ管理のドキュメントにクリーンページとダーティページの管理に関する情報を追加した。

- `インストーラー`: 圧縮戦略の最適化により、Windows のインストーラーにかかる時間が最大 60% 短縮されました。
    (UUM-136633)



## API の変更

- `エンティティグラフィックス`: 非推奨：Entities GraphicsのOpenGL ESのサポートは非推奨です。グラフィックス API 自体は有効な選択肢のままですが、Entities Graphics パッケージは将来のバージョンの Unity ではサポートされなくなります。

- `iOS`: 追加: iPhone 17e デバイス世代の列挙とスクリーンカットアウトを追加しました。



## 修正

- `2D`: タイルパレットのブラシピックオーバーレイが拡張されたとき、ブラシピックリストのアイテムが最小サイズより低い高さで初期化される問題を修正。
    ([uum-132593](https://issuetracker.unity3d.com/issues/saved-picks-list-items-are-initialised-with-a-lower-height-than-the-minimum-size-when-the-tile-palette-brush-pick-overlay-is-expanded))

- `2D`: ブラシピックリストのアイテムが不正な形になり、スライダーでピックサイズを小さくしたときにリストが切れてしまう問題を修正しました。
    ([uum-132606](https://issuetracker.unity3d.com/issues/saved-tile-palette-brush-list-items-become-malformed-causing-the-list-to-become-cut-off-when-decreasing-the-pick-size-with-the-slider))

- `2D`: ピックのサイズを調整し、表示モードをグリッドに切り替えた後、ブラシピックの名前が消える問題を修正。
    ([uum-132598](https://issuetracker.unity3d.com/issues/saved-tile-palette-brush-names-disappear-after-adjusting-the-size-of-the-picks-and-switching-view-mode-to-grid))

- `2D`: 2つ目のタイルパレットブラシピックアセットを作成する際に、データ損失エラーを回避するためにアセットの破棄が許可されていない問題を修正。
    ([uum-132608](https://issuetracker.unity3d.com/issues/destroying-assets-is-not-permitted-to-avoid-data-loss-error-thrown-when-creating-a-second-tile-palette-brush-pick-asset))

- `2D`: ブラシピックを大量に保存した場合に、ブラシピックリストがスクロールしてしまう問題を修正。
    ([uum-134369](https://issuetracker.unity3d.com/issues/scrolling-in-the-tile-palette-brush-pick-overlay-breaks-when-a-lot-of-brushes-are-saved-and-the-list-view-is-enabled))

- `2D`: タイルパレットを作成する際に、セルサイズプロパティが負の値を受け入れていた問題を修正しました。エディターは負の値を0に設定するようになりました。
    ([uum-130884](https://issuetracker.unity3d.com/issues/tile-palette-allows-negative-manual-cell-size-values-causing-asset-corruption-and-assertion-failed-on-expression-errors))

- `2D`: シャドウキャスターの2Dカリングを修正。
    ([uum-135640](https://issuetracker.unity3d.com/issues/2d-freeform-light-shadows-disappear-when-light-center-is-offset-from-shape-bounds))

- `適応パフォーマンス`: 長い名前がエディタをフリーズさせるのを避けるため、スケーラープロファイル名の長さに制限(256文字)を追加。
    ([uum-136422](https://issuetracker.unity3d.com/issues/the-editor-freezes-indefinitely-when-copy-pasting-large-amounts-of-characters-into-new-scaler-profile-name-field-in-the-adaptive-performance-settings))

- `アダプティブ・パフォーマンス`: Adaptive Performanceの*Logging Frequency*設定が負の値を受け付けないように修正。
    ([uum-133890](https://issuetracker.unity3d.com/issues/adaptive-performance-logging-frequency-can-be-set-to-a-negative-number))

- `アニメーション`: アバターが無効なハンドボーンを持っていた場合のクラッシュを修正。
    ([uum-136330](https://issuetracker.unity3d.com/issues/crash-on-mecanim-skeleton-skeletonsetdof-when-entering-play-mode-in-a-specific-scene-1))

- `ドキュメント`: UnityEngine.Cache クラスの 4GB 制限に関する記述を削除しました。AssetBundles をダウンロードする際のキャッシュの最大サイズは、Cache.maxAvailableStorageSpace 値または物理ディスク容量のいずれか低い方によって制限されます。設定されていない場合、値は使用可能なディスク容量になります。
    (UUM-129965)

- `DX12`: CommandBuffer.SetRenderTarget`が無効な深度スライスを使用した場合に、レンダーターゲットのバインディング中にクラッシュする問題を修正しました。
    (UUM-130121)

- `DX12`: 以前はコンピュートシェーダーのSRVとしてバインドされていたレンダーターゲットへの描画を再開した後に、リソースバリアが欠落していたのを修正しました。
    (UUM-127520)

- `Editor`: アダプティブ・パフォーマンス・モードをプレイ・モード中に切り替えることができなくなりました。
    ([UUM-133323](https://issuetracker.unity3d.com/issues/the-referenced-script-unknown-on-this-behaviour-is-missing-warnings-and-scriptablesingleton-already-exists-errors-are-thrown-when-adaptive-performance-is-disabled-in-play-mode))

- `Editor`: 読みやすさのために、Animatorウィンドウのテキストを常にSDF(Signed Distance Field)レンダリングを使用するように変更。
    ([uum-134486](https://issuetracker.unity3d.com/issues/animator-state-labels-are-not-rendered-correctly-when-using-the-bitmap-renderer-as-the-default-editor-text-renderer))

- `Editor`: BuildProfileの重複を括弧で区別した。
    (UUM-130365)

- `エディター`: エディターがバッチモードで起動されている場合、シーンバックアップの回復をオフにしない。
    (UUM-136292)

- `エディター`: エディターがプレハブインスタンスから `SerializeReference` に関連する未使用のオーバーライドを正しくクリアするようになりました。
    ([UUM-135932](https://issuetracker.unity3d.com/issues/prefabs-remove-unused-overrides-fails-to-clean-up-overrides-when-the-managed-instance-type-in-serializereference-is-changed))

- `エディター`: 特定のケースで発生する4フレームメモリアロケータ警告を修正。
    ([uum-86868](https://issuetracker.unity3d.com/issues/a-memory-leak-occurs-when-selecting-the-console-window-and-the-inspector-window-covers-the-game-view-window-when-specific-ui-elements-are-displayed-in-it))

- `エディター`: USSスタイルのバリデーションで、タブが空白であることを認識していなかった問題を修正した。
    ([uum-135915](https://issuetracker.unity3d.com/issues/warning-expected-end-of-value-but-found-is-shown-when-inserting-a-blank-line-between-opacity-and-scale-in-a-uss-file))

- `エディター`: Linuxでカーソルアイコンの状態が正しくなかった問題を修正。
    ([uum-112638](https://issuetracker.unity3d.com/issues/mouse-cursor-does-not-revert-to-the-default-mouse-cursor-when-pressing-the-add-column-button-in-the-search-window))

- `エディター`: ATGの右から左への入力で、端の削除とバックスペースに関する問題を修正した。
    ([uum-134654](https://issuetracker.unity3d.com/issues/ui-toolkit-backspace-tries-to-delete-the-character-to-the-left-of-the-pointer-when-the-language-direction-is-set-to-rtl-and-the-text-generator-type-is-set-to-advanced))

- `エディター`: Linuxでレイアウトをリセットした際にエディターウィンドウの寸法が保持されない問題を修正。
    (UUM-131233)

- `エディター`: Windows、Mac、Linux*で*Overrideを切り替えた後、Player Settingsのアイコンのサイズと保持が保持されない問題を修正しました。
    ([UUM-131965](https://issuetracker.unity3d.com/issues/icon-override-texture2d-fields-in-the-player-settings-window-are-not-square-and-do-not-retain-data-after-disabling))

- `エディター`: プロジェクト設定ウィンドウで検索フィルターを削除すると、そのセクションのテキストが完全にハイライトされてしまう問題を修正しました。
    ([uum-134769](https://issuetracker.unity3d.com/issues/all-text-in-the-project-settings-window-is-still-tinted-white-after-deleting-text-in-the-search-window))

- `エディター`: レガシーアニメーションクリップのラップモードオプションが正しく表示されない問題を修正しました。
    (UUM-135008)

- `エディター`: Linux エディターで、同じ名前のファイルを上書きする際にプロンプトが表示されない問題を修正。
    ([UUM-134937](https://issuetracker.unity3d.com/issues/linux-no-file-already-exists-pop-up-thrown-when-saving-shader-variants-with-an-already-existing-name-from-build-profiles-or-project-settings-window))

- `エディター`: AddComponentMenu` で、似たようなパススライスを持つアイテムが正しくグループ化されず、代わりに独自のカテゴリが与えられてしまう問題を修正しました。
    ([uum-135817](https://issuetracker.unity3d.com/issues/physics-components-are-ungrouped-in-components-menu-and-each-physics-component-shown-separately-in-inspector-window-after-installing-unity-physics-package))

- `エディター`: FormerlySerializedAsが型のフィールドと衝突する場合に、プレハブのプロパティ修正を適用するように修正。
    ([uum-135039](https://issuetracker.unity3d.com/issues/object-reference-is-saved-on-an-incorrect-property-when-formerlyserializedas-is-used-on-an-overridden-prefab-variant-field))

- `エディター`: UIBuilderプレビューのATG警告を修正。
    ([uum-136362](https://issuetracker.unity3d.com/issues/an-error-is-thrown-when-using-a-static-font-and-editor-text-generator-type-is-set-to-advanced))

- `エディター`: TextCoreの日中韓改行を修正。
    ([uum-135887](https://issuetracker.unity3d.com/issues/cjk-text-goes-outside-of-the-bounds-of-a-label-in-ui-builder-when-wrapping-is-enabled))

- `エディター`: ビルドプロファイルウィンドウで、エラーメッセージがインストールされた環境を正しく反映しない問題を修正。
    (UUM-112981)

- `エディター`: TextCoreでCJKを使用するとクラッシュする問題を修正。
    ([UUM-136683](https://issuetracker.unity3d.com/issues/editor-freezes-when-rendering-wrapped-text-inside-japanese-parentheses-in-too-small-of-a-space))

- `エディター`: 標準テキストで太字スタイルを使用したときにグローバルフォールバックが見つからない問題を修正。
    ([uum-136612](https://issuetracker.unity3d.com/issues/synthetic-bold-applies-to-a-label-instead-of-bold-font-weight-when-using-fallback-font-asset))

- `エディター`: グラフィック設定" セクションのラベルがずれていたのを修正。
    (UUM-135081)

- `エディター`: MacEditor でマウスをドラッグし続けている間、SceneView Maximize ショートカットがビューを復元しないのを修正。
    ([UUM-134463](https://issuetracker.unity3d.com/issues/scene-view-cant-be-minimised-using-the-shortcut-when-it-was-maximised-using-a-shortcut-in-the-flythrough-mode))

- `エディター`: モジュール詳細パネルのコントローラースクリプト例を修正。

- `エディター`: スクリプトの定義がコードで変更されても、UIから手動で適用する必要がなくなりました。
    ([uum-129228](https://issuetracker.unity3d.com/issues/build-profiles-modified-by-script-require-manual-apply-when-the-build-profiles-window-is-open))

- `エディター`: Scripting Definesの "Copy Defines "ボタンが、選択されたリストから定義を適切にコピーするようになりました。
    (UUM-133917)

- `Editor`: Adaptive Performance 設定ファイルにインスペクタ UI からアクセスできなくなりました。
    ([UUM-133313](https://issuetracker.unity3d.com/issues/keys-and-values-lists-in-the-adaptive-performance-general-settings-have-plus-buttons-that-are-active-but-do-nothing-when-clicked))

- `Editor`: アダプティブ・パフォーマンス設定ファイルにインスペクターUIからアクセスできなくなりました。
    ([uum-133314](https://issuetracker.unity3d.com/issues/adaptive-performance-general-settings-version-property-is-set-to-6000-dot-3-on-all-unity-editor-versions))

- `エディター`: UI Toolkit テストフレームワーク: EditorWindowUITestFixture を修正し、テスト中のドメインリロードをサポートするようにしました。
    (UUM-136644)

- `エンジン診断`: Diagnostics ネイティブクラッシュレポートの誤ったタイムスタンプを修正。
    (UUM-135436)

- `エンジン診断`: usymtool が一部のログ出力を欠落させていた問題を修正しました。
    (UUM-136272)

- `GI`: xAtlas lightmap packer を一部の不正なメッシュで使用するとクラッシュする問題を修正。
    ([UUM-133550](https://issuetracker.unity3d.com/issues/crash-when-generating-lighting-using-the-xatlas-packing-method-in-a-specific-scene))

- `GI`: xAtlas TaskSchedulerワーカースレッドのハングを修正しました。
    (UUM-133340)

- `GI`: xAtlas TaskScheduler ワーカースレッドのハングを修正。
    (UUM-133352)

- `Graphics`: NVIDIA ドライバのアップデート後に SystemInfo.supportsInlineRayTracing が false になる問題を修正しました。
    ([UUM-137132](https://issuetracker.unity3d.com/issues/raytracing-updating-nv-graphics-drivers-to-595-dot-79-causes-systeminfo-dot-supportsinlineraytracing-to-become-false))

- `Graphics`: 一部の Android デバイスでインスタンス化を使用した際の地形の継ぎ目を修正しました。
    (UUM-131277)

- `Graphics`: サブパスの最大数の内部制限を16に増加。
    ([UUM-128596](https://issuetracker.unity3d.com/issues/number-of-srp-renderpass-subpasses-is-limited-to-8))

- `Graphics`: builtinRP blit copy が float4 カラーを出力するようにした(fxed4 はメタル上では half4 に変換されるため、u16 または f32 データではデータが切り捨てられる)。
    ([uum-128591](https://issuetracker.unity3d.com/issues/terrain-heightmap-loses-precision-on-android-and-ios-when-the-heightmap-texture-is-blitted-to-a-rendertexture-using-the-default-blit-shader))

- `HDRP`: カスタムパスは、nonMSAAColorBuffer と colorBuffer が同じテクスチャを指している場合、両方を読み込まない。
    ([uum-133557](https://issuetracker.unity3d.com/issues/game-view-turns-black-with-render-graph-and-invalidoperationexception-errors-when-forward-only-lit-shader-mode-and-msaa-are-enabled))

- `HDRP`: VolumeProfileがいくつかのケースで全てのコンポーネントタイプの収集に失敗する問題を修正しました。
    (UUM-136181)

- `Input`: IOHIDDevice_GetUsage の無効なロングバッファポインタを修正しました。
    ([UUM-135043](https://issuetracker.unity3d.com/issues/silicon-crash-on-os-unfair-recursive-lock-lock-with-options-when-opening-a-project-while-the-razer-synapse-app-is-open))

- `iOS`: アプリの実行中にカスタム URL スキームまたはユニバーサルリンク経由でアプリを開いた場合、またはコールドスタートからディープリンク経由でアプリを起動した場合に、Application.deepLinkActivated が呼び出されない問題を修正しました。
    ([uum-135497](https://issuetracker.unity3d.com/issues/ios-application-dot-deeplinkactivated-does-not-get-invoked-while-app-is-running-when-uiapplicationscenemanifest-is-added-in-info-dot-plist))

- `iOS`: キーボードレイアウトが変更された場合のみ報告することでパフォーマンスを改善しました。
    ([uum-134845](https://issuetracker.unity3d.com/issues/ios-high-cpu-load-when-device-keyboard-is-open))

- `ライセンス`: 正確な結果を得るために Package Manger からのクエリに対してエンタイトルメントキャッシュをバイパスするようにしました。

- `macOS`: 終了時にクラッシュすることがあったのを修正。
    ([uum-136306](https://issuetracker.unity3d.com/issues/crash-slash-freeze-when-quitting-macos-standalone-player-that-is-actively-rendering-with-metal))

- `マルチプレイヤー`: プレイモードに入った時にエディターロールがMPPMプレイモードのシナリオロールと同期しない問題を修正。
    ([uum-136438](https://issuetracker.unity3d.com/issues/editor-role-does-not-sync-with-the-mppm-play-mode-scenario-role-when-entering-play-mode))

- `ネットワーキング`: UnityWebRequest の httpForcedVersion プロパティを公開し、必要に応じてネットワーク通信に特定の http プロトコルバージョンを使用させることができるようにしました。
    (UUM-131759)

- `物理`: ForceMode.Accelerationによって加速度が適用されるRigidbodyが、回転を拘束されている場合に、NaN値が発生することがある問題を修正しました。この問題は、Physics.Simulate を手動で呼び出した場合によく発生します。
    (UUM-103515)

- `物理`: MeshCollider`で`Use Fast Midphase`クッキングオプションを使用してレイキャストを行う際に、スタックオーバーフローを防止しました。
    ([UUM-110564](https://issuetracker.unity3d.com/issues/silent-crash-when-raycasting-on-a-specific-dense-mesh))

- `物理 2D`: すべての物理オブジェクトと引数の検証が最終的なプレーヤのビルドで行われるようになり、一貫性のある動作が保証されました。
    ([uum-137011](https://issuetracker.unity3d.com/issues/physicscore2d-internal-validation-doesn-t-happen-in-non-dev-player-build))

- `物理 2D`: PhysicsWorld.TestOverlapShape、PhysicsWorld.OverlapShape、PhysicsWorld.CastShape、PhysicsShape.CastShape が、ワールド空間での結果を正しく返すようになり、期待通りに元のシェイプを無視するようになりました。
    (UUM-136473)

- `物理 2D`: ChainSegmentGeometry の直列化フィールドを再配置し、インスペクタでより直感的な順序で表示されるように。
    (UUM-137108)

- `QNX`: QNX ウィンドウのタッチ/マウス入力座標を修正。
    (UUM-135196)

- `Shadergraph`: ノードのコピーペーストや複製を行うと、すべてのノードのプレビューが元の状態を尊重するのではなく、環境設定に基づいて折りたたまれたり拡大されたりする問題を修正。
    (UUM-136522)

- `Shadergraph`: Preference がデフォルトでプレビューを展開しないように設定されている場合に、プレビューノードが折りたたまれた状態で作成される問題を修正しました。
    (UUM-136521)

- `Shadergraph`: エラーと警告のノードバッジツールチップがコンテナから溢れるのを修正。
    ([UUM-133930](https://issuetracker.unity3d.com/issues/validation-message-text-overflows-its-background-in-shader-graph-custom-function-node))

- `シェーダー`: ビルドプロセス中に稀にクラッシュする問題を修正。
    ([uum-134784](https://issuetracker.unity3d.com/issues/crash-on-shadercompilation-transientdata-strippasses-when-building-a-project-with-specific-graphics-settings))

- `シェーダー`: フロー制御の環境設定フラグを設定して、シェーダーのコンパイルの警告をエラーの代わりに報告するように。
    ([uum-130622](https://issuetracker.unity3d.com/issues/compute-shaders-throw-compilation-errors-when-show-compiled-code-is-pressed))

- `テキスト`: リンクタグがヘルプボックスで機能するように。
    ([uum-135690](https://issuetracker.unity3d.com/issues/textgenerationinfo-pointer-is-null-dot-errors-are-spammed-and-link-doesnt-work-when-clicking-on-the-hyperlinks-in-the-build-profiles))

- `テキスト`: テキストの中央揃えと右揃えで文字間隔が正しく機能しない問題を修正。
    ([uum-134277](https://issuetracker.unity3d.com/issues/letters-on-the-left-side-move-further-as-letter-spacing-is-increased-when-advanced-text-generator-is-used-and-alignment-is-set-to-center))

- `テキスト`: ATGのキャッシュポイズニング問題を修正。
    ([uum-134255](https://issuetracker.unity3d.com/issues/uitoolkit-editor-window-text-is-not-displayed-when-using-advanced-text-generation))

- `テキスト`: ATG のキャッシュ問題を修正。
    ([uum-135642](https://issuetracker.unity3d.com/issues/text-after-a-unicode-private-use-area-character-is-invisible-when-advanced-text-generator-is-used))

- `テキスト`: 標準テキストでビットマップを使用するとハイライトが不正になる問題を修正。
    ([uum-134691](https://issuetracker.unity3d.com/issues/project-settings-search-highlights-are-misaligned-when-using-the-bitmap-text-rendering-mode))

- `uGUI`: フォント再構築のスコープ中にテキストコンポーネントが追加された場合の例外を修正。
    ([uum-128580](https://issuetracker.unity3d.com/issues/invalidoperationexception-thrown-when-text-legacy-font-is-rebuilt-via-inputfield-legacy-onvaluechanged-callback))

- `UIエレメント`: UnityEventのインスペクタのコピー/ペーストを修正し、CustomEditorを持つコンポーネント(Buttonなど)と持たないコンポーネントの間で動作するようにしました。以前は、コピーしたUnityEventをこれらの2種類のコンポーネント間で貼り付けると、互換性のないクリップボードフォーマットのために静かに失敗していました。
    ([uum-134401](https://issuetracker.unity3d.com/issues/unityevent-does-not-get-pasted-to-a-ugui-component-when-trying-to-copy-it-from-a-script))

- `UIツールキット`: 他のプレビューに影響を与えることなくUITKPreviewをサポートするように修正。
    ([uum-133704](https://issuetracker.unity3d.com/issues/shadergraph-only-uv0-displays-a-propre-preview))

- `UIツールキット`: EditorUtility.activePlayModeTintを公開し、カスタムメッシュやPainter2Dの色がエディタと一貫して見えるように。
    (UUM-114424)

- `UI ツールキット`: オーバーフローまたはコーナー半径を変更した後に不正なクリッピングを引き起こす可能性のあるバグを修正しました。
    ([UUM-135869](https://issuetracker.unity3d.com/issues/invalid-clip-method))

- `UIツールキット`: Play モードで SVG アセットをインスペクションする際に表示される、`No Theme Style Sheet set to PanelSettings` という警告を修正しました。
    ([uum-123228](https://issuetracker.unity3d.com/issues/no-theme-style-sheet-set-to-panelsettings-warnings-appear-when-changing-tessellation-mode-of-an-svg-asset-used-as-a-background-of-visual-element))

- `UIツールキット`: フィルターのステンシルマスクが通常のジオメトリとしてレンダリングされる問題を修正しました。
    ([uum-135894](https://issuetracker.unity3d.com/issues/ui-toolkit-renders-visual-element-white-when-the-overflow-property-is-set-to-hidden-any-border-radius-is-applied-and-any-filter-is-applied))

- `UIツールキット`: 現在の値がスタイルシートから継承されている場合に `FilterStyleField` のインライン値を編集する際の問題を修正しました。
    (UUM-135010)

- `UIツールキット`: コマンドリストのリークを修正しました。
    ([UUM-135928](https://issuetracker.unity3d.com/issues/ui-toolkit-leaks-4-persistant-allocates-when-the-displaystyle-of-a-visualelement-is-set-from-flex-to-none))

- `UIツールキット`: カスタムシェーダー使用時のエディターウィンドウのサンプル要素テクスチャーノードのガンマ問題を修正。
    ([uum-135360](https://issuetracker.unity3d.com/issues/ui-toolkit-visualelements-render-darker-when-a-custom-shader-graph-material-is-assigned-in-ui-builder))

- `UIツールキット`: フラットUI(オーバーレイまたはエディタ)でZの保存に矛盾があったのを修正。
    ([uum-115032](https://issuetracker.unity3d.com/issues/dragging-search-query-block-can-remove-its-color))

- `UI Toolkit`: MultiColumnTreeViewで、ソート時にGetIdFromIndexとGetItemDataForIndexから間違ったデータを返す問題を修正しました。
    (UUM-111055)

- `UIツールキット`: プレイモードの色合いが常に適用されない問題を修正。
    (UUM-136707)

- `UIツールキット`: UI Toolkit シェーダーのプレビューコード生成を修正。
    ([UUM-130869](https://issuetracker.unity3d.com/issues/ui-toolkit-custom-shaders-shows-incorrect-previews-for-preview-and-combine-nodes-when-inputting-the-default-solid-b-channel))

- `UI Toolkit`: EditorMonitor の RenderTexture リークを修正。
    (UUM-104256)

- `UIツールキット`: 生成されたアセットタイプを切り替えた後、SVGテッセレーションモードが持続しない問題を修正。
    ([UUM-136307](https://issuetracker.unity3d.com/issues/tessellation-mode-changes-when-switching-svg-generated-asset-type-back-to-ui-toolkit-vector-image))

- `UIツールキット`: SVGImporterがいくつかのインポートされたファイルからシリアライズされたFileIDを保持しない問題を修正。
    ([uum-136228](https://issuetracker.unity3d.com/issues/svg-image-component-loses-source-svg-image-field-references-when-upgrading-to-a-different-vector-graphics-package-version))

- `UI Toolkit`: WebGL シェーダーの精度に関する問題を修正しました。
    ([uum-132006](https://issuetracker.unity3d.com/issues/ui-toolkit-text-becomes-visually-distorted-when-the-text-is-not-directly-facing-the-camera))

- `UI Toolkit`: オーバーレイ UI のレンダリング時に invertCulling を false に強制することで、リーク状態に対する安全性を強化。
    (UUM-136734)

- `VFX Graph`: ソート計算パスによって引き起こされるiOSでのレンダリングアーチファクトとフリーズを修正。
    (UUM-136074)

- `ビデオ`: WebGLSupport` の fakemod モジュールの `ChannelGroupI::getPitch` の警告を修正。
    ([UUM-134014](https://issuetracker.unity3d.com/issues/fmod-returns-error-code-78-message-repeatedly-shown-in-the-console-when-playing-audio-using-an-audio-random-container))

- `WebGL`: WebGPU：一部のシェーダーがコンパイルに失敗する原因となっていたシェーダーコンパイラのエラーを修正しました。
    ([uum-134978](https://issuetracker.unity3d.com/issues/shader-compiler-and-shader-errors-are-thrown-in-the-console-when-building-web-platform-with-webgpu-experimental-enabled-in-auto-graphics-api))




## 6000.3.12f1 におけるパッケージの変更

## 更新されたパッケージ

- `com.unity.collections`: [2.6.2](https://docs.unity3d.com/Packages/com.unity.collections@2.6//changelog/CHANGELOG.html) から [2.6.5](https://docs.unity3d.com/Packages/com.unity.collections@2.6//changelog/CHANGELOG.html) へ。

- `com.unity.entities`: [1.4.2](https://docs.unity3d.com/Packages/com.unity.entities@1.4//changelog/CHANGELOG.html) から [1.4.5](https://docs.unity3d.com/Packages/com.unity.entities@1.4//changelog/CHANGELOG.html)

- `com.unity.physics`: [1.4.2](https://docs.unity3d.com/Packages/com.unity.physics@1.4//changelog/CHANGELOG.html) から [1.4.5](https://docs.unity3d.com/Packages/com.unity.physics@1.4//changelog/CHANGELOG.html)

- `com.unity.entities.graphics`: [1.4.15](https://docs.unity3d.com/Packages/com.unity.entities.graphics@1.4//changelog/CHANGELOG.html) から [1.4.18](https://docs.unity3d.com/Packages/com.unity.entities.graphics@1.4//changelog/CHANGELOG.html) まで。

- `com.unity.cinemachine`: [2.10.6](https://docs.unity3d.com/Packages/com.unity.cinemachine@2.10//changelog/CHANGELOG.html) から [2.10.7](https://docs.unity3d.com/Packages/com.unity.cinemachine@2.10//changelog/CHANGELOG.html) へ。

- `com.unity.performance.profile-analyzer`: [1.3.2](https://docs.unity3d.com/Packages/com.unity.performance.profile-analyzer@1.3//changelog/CHANGELOG.html) から [1.3.3](https://docs.unity3d.com/Packages/com.unity.performance.profile-analyzer@1.3//changelog/CHANGELOG.html) に変更。

- `com.unity.postprocessing`: [3.5.1](https://docs.unity3d.com/Packages/com.unity.postprocessing@3.5//changelog/CHANGELOG.html) から [3.5.4](https://docs.unity3d.com/Packages/com.unity.postprocessing@3.5//changelog/CHANGELOG.html) に変更。

- `com.unity.services.analytics`: [6.2.2](https://docs.unity3d.com/Packages/com.unity.services.analytics@6.2//changelog/CHANGELOG.html) から [6.3.0](https://docs.unity3d.com/Packages/com.unity.services.analytics@6.3//changelog/CHANGELOG.html)

- `com.unity.splines`: [2.8.3](https://docs.unity3d.com/Packages/com.unity.splines@2.8//changelog/CHANGELOG.html) から [2.8.4](https://docs.unity3d.com/Packages/com.unity.splines@2.8//changelog/CHANGELOG.html) へ。

- `com.unity.visualscripting`: [1.9.10](https://docs.unity3d.com/Packages/com.unity.visualscripting@1.9//changelog/CHANGELOG.html) から [1.9.11](https://docs.unity3d.com/Packages/com.unity.visualscripting@1.9//changelog/CHANGELOG.html) へ。

- `com.unity.multiplayer.playmode`: [2.0.1](https://docs.unity3d.com/Packages/com.unity.multiplayer.playmode@2.0//changelog/CHANGELOG.html) から [2.0.2](https://docs.unity3d.com/Packages/com.unity.multiplayer.playmode@2.0//changelog/CHANGELOG.html) へ。

- `com.unity.transport`: [2.6.0](https://docs.unity3d.com/Packages/com.unity.transport@2.6//changelog/CHANGELOG.html)から[2.7.1](https://docs.unity3d.com/Packages/com.unity.transport@2.7//changelog/CHANGELOG.html)

- `com.unity.dedicated-server`: [2.0.1](https://docs.unity3d.com/Packages/com.unity.dedicated-server@2.0//changelog/CHANGELOG.html) から [2.0.2](https://docs.unity3d.com/Packages/com.unity.dedicated-server@2.0//changelog/CHANGELOG.html)

- `com.unity.memoryprofiler`: [1.1.11](https://docs.unity3d.com/Packages/com.unity.memoryprofiler@1.1//changelog/CHANGELOG.html) から [1.1.12](https://docs.unity3d.com/Packages/com.unity.memoryprofiler@1.1//changelog/CHANGELOG.html) へ。

- `com.unity.netcode.gameobjects`: [2.9.1](https://docs.unity3d.com/Packages/com.unity.netcode.gameobjects@2.9//changelog/CHANGELOG.html) から [2.10.0](https://docs.unity3d.com/Packages/com.unity.netcode.gameobjects@2.10//changelog/CHANGELOG.html)

- `com.unity.polyspatial`: [3.1.0](https://docs.unity3d.com/Packages/com.unity.polyspatial@3.1//changelog/CHANGELOG.html)から[3.1.1](https://docs.unity3d.com/Packages/com.unity.polyspatial@3.1//changelog/CHANGELOG.html)

- `com.unity.polyspatial.visionos`: [3.1.0](https://docs.unity3d.com/Packages/com.unity.polyspatial.visionos@3.1//changelog/CHANGELOG.html) から [3.1.1](https://docs.unity3d.com/Packages/com.unity.polyspatial.visionos@3.1//changelog/CHANGELOG.html)

- `com.unity.polyspatial.xr`: [3.1.0](https://docs.unity3d.com/Packages/com.unity.polyspatial.xr@3.1//changelog/CHANGELOG.html) から [3.1.1](https://docs.unity3d.com/Packages/com.unity.polyspatial.xr@3.1//changelog/CHANGELOG.html) に変更。

- `com.unity.polyspatial.extensions`: [3.1.0](https://docs.unity3d.com/Packages/com.unity.polyspatial.extensions@3.1//changelog/CHANGELOG.html) から [3.1.1](https://docs.unity3d.com/Packages/com.unity.polyspatial.extensions@3.1//changelog/CHANGELOG.html) に変更。

- `com.unity.xr.visionos`: [3.1.0](https://docs.unity3d.com/Packages/com.unity.xr.visionos@3.1//changelog/CHANGELOG.html) から [3.1.1](https://docs.unity3d.com/Packages/com.unity.xr.visionos@3.1//changelog/CHANGELOG.html) へ。

- `com.unity.asset-manager-for-unity`: [1.9.2](https://docs.unity3d.com/Packages/com.unity.asset-manager-for-unity@1.9//changelog/CHANGELOG.html)から[1.10.0](https://docs.unity3d.com/Packages/com.unity.asset-manager-for-unity@1.10//changelog/CHANGELOG.html)

- `com.unity.cloud.draco`: [5.4.2](https://docs.unity3d.com/Packages/com.unity.cloud.draco@5.4//changelog/CHANGELOG.html) -> [5.4.3](https://docs.unity3d.com/Packages/com.unity.cloud.draco@5.4//changelog/CHANGELOG.html)

- `com.unity.cloud.ktx`: [3.6.2](https://docs.unity3d.com/Packages/com.unity.cloud.ktx@3.6//changelog/CHANGELOG.html) から [3.6.3](https://docs.unity3d.com/Packages/com.unity.cloud.ktx@3.6//changelog/CHANGELOG.html) に変更。