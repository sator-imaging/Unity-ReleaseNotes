...
Published at Wed, 03 Dec 2025 16:52:53 GMT  
...
- 2D: Fixed an issue where 2D shadows could disappear after a domain reload.
    ([UUM-115234](https://issuetracker.unity3d.com/issues/shadow-caster-2d-shadows-partially-disappear-in-scene-view-when-domain-is-reloaded))
...
- Android: Fixed a visible icon artefact during the splash screen on Android XR.
    (UUM-126180)
...
- Animation: Fixed a crash when deleting a playable output with a null graph.
...
- Asset Pipeline: Fixed an issue where files disappeared in the Project window when reimporting a folder.
    (UUM-126423)
...
- Audio: Fixed an issue where the Inspector displayed an incorrect link when referencing `AudioSnapshot`.
...
- Editor: Fixed "Unity Services are not disabled and cause longer build times when the project is not connected to Unity Cloud" warning.
...
- Editor: Fixed a AudioRandomContainer loop count randomisation UI flickering when changing values through range slider.
    ([UUM-115219](https://issuetracker.unity3d.com/issues/audio-random-container-randomisation-fields-and-slider-flickers))<br>
...
- Editor: Fixed a crash that would occur when some engine textures are used during shadow pass.
...
- Editor: Fixed a thread issue when using matchMaterialPreset.
...
- Editor: Fixed an Editor crash that would occur when loading single byte asset meta files.
...
- Editor: Fixed an issue where a Raycast did not hit when using the default `CapsulecastCommand()` constructor.
    ([UUM-123124](https://issuetracker.unity3d.com/issues/no-valid-hits-are-returned-when-using-raycastcommand))

- Editor: Fixed an issue where a Raycast did not hit when using the default `RaycastCommand()` constructor.
    ([UUM-123124](https://issuetracker.unity3d.com/issues/no-valid-hits-are-returned-when-using-raycastcommand))

- Editor: Fixed an issue where a Raycast did not hit when using the default `SpherecastCommand()` constructor.
    ([UUM-123124](https://issuetracker.unity3d.com/issues/no-valid-hits-are-returned-when-using-raycastcommand))

- Editor: Fixed an issue where dialogs in the Windows Editor were obscured by secondary windows.
    ([UUM-125650](https://issuetracker.unity3d.com/issues/when-tile-palette-dialog-boxes-lose-focus-the-editor-becomes-uninteractable-and-the-dialog-box-hides-behind-the-tile-palette-window))<br>
...
- Editor: Prevented null asset from being passed to `OnWillSaveAssets`.
...
- IL2CPP: Fixed an issue where IL2CPP randomly crashing with exit code 139 on macOS arm64.
    (UUM-125068)
...
- iOS: Fixed an issue where the touch keyboard on iOS reported it was visible after dismissal.
...
- Package Manager: Fixed keeping text in fields when adding a package by technicial name fails.
    ([UUM-125060](https://issuetracker.unity3d.com/issues/add-package-by-technical-name-fields-are-emptied-when-an-invalid-value-is-entered))<br>
    *First seen in 6000.3.0b8.*
...
- Physics 2D: Add missing methods to retrieve all geometry-islands for PhysicsComposer and PhysicsDestructor operations.
    ([UUM-126432](https://issuetracker.unity3d.com/issues/no-way-to-get-geometry-islands-from-physics-composer-or-destructors))<br>
    *First seen in 6000.3.0a5.*
...
- Physics 2D: Fixed a crash when using PhysicsWorld Transform Tweening.
    ([UUM-126195](https://issuetracker.unity3d.com/issues/crash-on-block-merge-next-when-updating-the-transform-of-a-physicsbody))<br>
    *First seen in 6000.3.0b8.*

- Profiler: Fixed "Unable to find style" warning on switching color blind mode on or off.
    ([UUM-117113](https://issuetracker.unity3d.com/issues/unable-to-find-style-warnings-thrown-when-selecting-memory-module-and-enabling-color-blind-mode))<br>
    *First seen in 6000.3.0a1.*
...
- Shaders: Fixed a regression issue when triggering keyword space mismatches on certain cases with UsePass and Fallback.
...
- Terrain: Fixed a crash that could occur when preventing instanced details from being added to detail texture atlas.
...
- TextCore: Fixed a thread issue with underline.
...
- UI Toolkit: Fixed color space bugs in filters
    ([UUM-119998](https://issuetracker.unity3d.com/issues/uss-filter-output-is-too-dark))<br>
...
- UI Toolkit: Fixed selecting an element selects all matching elements from other instances of the same document when unpacking instances of documents in UI Builder.
    ([UUM-125218](https://issuetracker.unity3d.com/issues/selecting-an-element-selects-all-matching-elements-from-other-instances-of-the-same-document-when-unpacking-instances-of-documents-in-ui-builder))<br>
    *First seen in 6000.3.0b7.*
...
- UI Toolkit: The Remove button in a `ListView` footer will be disabled when list is empty.
...
- UI Toolkit: UI Toolkit: Fixed blend mode for Additive and Multiply
    ([UUM-120002](https://issuetracker.unity3d.com/issues/uitk-custom-shader-blend-modes-bugs))<br>
    *First seen in 6000.3.0a2.*
...
- com.unity.inputsystem: [1.15.0](https://docs.unity3d.com/Packages/com.unity.inputsystem@1.15//changelog/CHANGELOG.html) to [1.16.0](https://docs.unity3d.com/Packages/com.unity.inputsystem@1.16//changelog/CHANGELOG.html)

- com.unity.xr.arcore: [6.3.0](https://docs.unity3d.com/Packages/com.unity.xr.arcore@6.3//changelog/CHANGELOG.html) to [6.3.1](https://docs.unity3d.com/Packages/com.unity.xr.arcore@6.3//changelog/CHANGELOG.html)

- com.unity.xr.arfoundation: [6.3.0](https://docs.unity3d.com/Packages/com.unity.xr.arfoundation@6.3//changelog/CHANGELOG.html) to [6.3.1](https://docs.unity3d.com/Packages/com.unity.xr.arfoundation@6.3//changelog/CHANGELOG.html)

- com.unity.xr.arkit: [6.3.0](https://docs.unity3d.com/Packages/com.unity.xr.arkit@6.3//changelog/CHANGELOG.html) to [6.3.1](https://docs.unity3d.com/Packages/com.unity.xr.arkit@6.3//changelog/CHANGELOG.html)
...
- 2D: Clean up and fixed mesh and sprite normals for shadergraph.
    (UUM-121472)
...
- 2D: Fixed an issue where Texture 2D Importer fails passing a default value to the "namefileidtable" when using a texture preset.
...
- Android: Fixed an issue in the build and run pipeline where the Android manifest was not correctly parsed for `activity-alias` entries. The Editor now supports launching applications that use `activity-alias` as an entry point.
...
- Android: Update minimum Android SDK build and platform tools versions to 36.0.0.
    (UUM-123164)
...
- Animation: Fixed a crash that occurred when playing animation clips from the Animation window on animators with invalid controllers that had no layers.
...
- Animation: Fixed crash caused by an avatar having invalid human bones.
...
- Asset Pipeline: Ensured that symlinked folders are correctly detected on Unix platforms and that a warning is displayed for each detected folder.
...
- Audio: Fixed an issue in the scriptable audio pipeline that affected speaker setups and channel counts.
    (DCME-1373)
...
- DX12: Fixed a RenderPass stencil access validation error \(ID 1377\).
    (UUM-122912)
...
- Editor: Add Scene GUI filtering similar to LODGroup to hide labels in non-active scene views for Mesh LOD labels.
    (GFXMLSA-1415)
...
- Editor: Burst `SharedStatic` memory is now cleared prior to object deserialization, which fixed a crash that could occur if jobs were scheduled during object deserialization.
    ([UUM-77850](https://issuetracker.unity3d.com/issues/crash-on-extracthandles-when-reloading-domain-in-a-specific-project))
...
- Editor: Fixed a BRG failing test on Android Samsung S21.
    (UUM-108763)
...
- Editor: Fixed a warning about debug symbols not being enabled for Cloud Diagnostics that could appear when the Cloud Diagnostics package wasn't installed.
...
- Editor: Fixed an issue in Build Profiles where script defines were applied without validation.
    (UUM-84855)

- Editor: Fixed an issue in the imgui's charField.
...
- Editor: Fixed an issue where entering Playmode with the Save Preset window open would throw an error in the console.
...
- Editor: Fixed an issue where sky contribution was missing in fog when sky occlusion was enabled.
...
- Editor: Fixed false warning that displayed *BatchRendererGroups currently don't support override shaders*.
...
- Editor: Remove Unused Overrides menu item in hierarchy is now disabled when scene is not loaded.
...
- Graphics: Disabled `CAMetalDisplayLink` on iOS to prevent potential GPU hangs and rendering freezes. It will be re-enabled once the underlying cause is identified and resolved.
...
- Graphics: Fixed an issue where ETC1 assets loaded from old AssetBundles appeared washed out when originally imported as sRGB and the project was set to use Linear color space.
...
- Graphics: Updated UX and documentation to reflect global behavior of Advanced Properties in SRP Inspectors.
...
- HDRP: Added a missing documentation link for the High Quality Line Renderer volume component in Graphics Settings.
...
- HDRP: Fixed an assertion that occurred when `HDCamera.allowDynamicResolution` was set to `true` via a script while using STP.
...
- HDRP: Fixed an issue where changing the intensity of a realtime area light via script or animation wouldn't affect the color of the light.
...
- Physics 2D: Fixed an issue with `Rigidbody2D` linear and angular drag to damping rename when used in prefab variants.
...
- TextMeshPro: Fixed an issue by revising style padding handling to exclude sprites.
    (UUM-102112)
...
- uGUI: Fixed a potential crash that occurred when a `CanvasRenderer` was destroyed while its parent canvas was nested under a disabled parent.
...
- uGUI: Updated the text color in the Intercepted Events Preview to match the Editor style.
...
- VFX Graph: Fixed a memory leak that occurred when using motion vectors.
    (UUM-122249)

- VFX Graph: Fixed a wrapping issue in the VFX Graph Control Panel.
...
- VFX Graph: Fixed an issue in the Visual Effect Graph where the window disappeared after using the Save As option with the same asset name. The window now remains open in this scenario.
    ([UUM-121821](https://issuetracker.unity3d.com/issues/saving-visual-effect-graph-closes-it-in-visual-effect-graph-window))

- VFX Graph: Fixed an issue where an exposed property value wasn't properly restored with multi-selection.
...
- VFX Graph: Fixed an issue where duplicated properties in the blackboard were not ordered correctly.
...
- VFX Graph: Fixed an issue where the preview was too bright in HDRP lit scenarios.
...
- VFX Graph: Fixed errors that occurred when undoing the action of moving a property into the output category in the Blackboard.
...
- VFX Graph: Fixed the output port order in `CustomHLSL` operators.
...
- VFX Graph: Prevented multiple outputs in the same graph from having the same shader name.
    (UUM-117582)
...
- VFX Graph: Restored panel visibility when opening the VFX Graph Editor and ensured that panels remain visible when entering Play mode.
